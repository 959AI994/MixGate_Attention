...
from .utils.attention_utils import AttentionAnalyzer  # 添加导入
...
class TopTrainer():
    def __init__(self,
        ...
        self.attention_analyzer = AttentionAnalyzer(save_dir=os.path.join(args.save_dir, 'attentions'))
        self.batch_counter = 0  # 初始化 batch 计数器
        self.save_every_n_batches = 26 # 每200个batch保存一次
    ...
    def save_attention(self, attention_info, batch_ids,circuit_names):
        """每50个batch保存一次注意力信息"""
        self.batch_counter += 1  # 每次调用时增加计数器
        ...
        if self.batch_counter % self.save_every_n_batches == 0:
            for batch_id in attention_info['attentions']:
                ...
                # 保存为压缩npz文件
                np.savez_compressed(
                    os.path.join(self.attention_analyzer.save_dir, f"{uid}.npz"),
    ...
    def run_batch(self, batch):
        ...
        # 新增：获取当前batch的所有电路名称
        circuit_names = batch.name  # 这里直接获取PyG batch的name属性
        
        mcm_pm_tokens, mask_indices, pm_tokens, pm_prob,aig_prob, mig_prob, xmg_prob, xag_prob,attention_info = self.model(batch)
        # 添加注意力保存逻辑
        if attention_info is not None:
            self.save_attention(attention_info, batch.batch.cpu().numpy(),circuit_names)
            # attention_info = None  # 直接将attention_info置为None，释放内存
            self.model.mask_tf.attention_buffer.clear()  # 清空mask_tf中的attention_buffer
            self.model.mask_tf.token_name_buffer.clear()  # 清空mask_tf中的token_name_buffer
        ...
        prob_xmgloss = self.reg_loss(xmg_prob, batch['prob'].unsqueeze(1))
...
#                     self.save(os.path.join(self.log_dir, 'model_last.pth'))
#                 if self.local_rank == 0:
#                         self.logger.write('{}| Epoch: {:}/{:} |Prob: {:.4f} |Func: {:.4f}|Prob_Aig: {:.4f} |Prob_Xmg: {:.4f} |Prob_Xag: {:.4f} |Prob_Mig: {:.4f}|Net: {:.2f}s \n'.format(
#                             phase, epoch, num_epoch, prob_loss_stats.avg,func_loss_stats.avg,prob_loss_aig.avg, prob_loss_mig.avg, prob_loss_xmg.avg, prob_loss_xag.avg,batch_time.avg))
#                         bar.finish()
