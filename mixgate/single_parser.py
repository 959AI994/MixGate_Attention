...
from .utils.circuit_utils import get_fanin_fanout, read_file, add_node_index, feature_gen_connect, get_hops
...
class NpzParser():
    ...
    class inmemory_dataset(InMemoryDataset):
        def __init__(self, root, circuit_path, transform=None, pre_transform=None, pre_filter=None):
            ...
            self.circuit_name = os.path.basename(circuit_path).split('.')[0]
        ...
        def process(self):
            data_list = []
            tot_pairs = 0
            circuits = read_npz_file(self.circuit_path)
            j = 0
            print('Parse circuit: {}'.format(self.circuit_name))
            
            # aig_prob_path = '/Users/zhengyuanshi/studio/MixGate_Attention/dataset/extracted_aig.npz'
            # aig = read_npz_file(aig_prob_path)['circuits'].item()[self.circuit_name]
            # aig_prob = aig['prob']
            # xmg_prob_path = '/Users/zhengyuanshi/studio/MixGate_Attention/dataset/extracted_xmg.npz'
            # xmg = read_npz_file(xmg_prob_path)['circuits'].item()[self.circuit_name]
            # xmg_prob = xmg['prob']
            
            x = circuits["xmg_x"]
            edge_index = circuits["xmg_edge_index"].T
            # is_pi = circuits["is_pi"]
            # no_edges = circuits["no_edges"]
            prob = torch.zeros(len(x), dtype=torch.float)
            # backward_level = circuits["backward_level"]
            # forward_index = circuits["forward_index"]
            # forward_level = circuits["forward_level"]
            # no_nodes = circuits["no_nodes"]
            # backward_index = circuits["backward_index"]
            tt_dis = torch.zeros(5, dtype=torch.float)
            tt_pair_index = torch.zeros([2, 5], dtype=torch.long)

            if len(tt_pair_index) == 0:
                raise ValueError('No tt : ', self.circuit_name)
            ...
            for modal in ['mig', 'aig', 'xmg', 'xag']:
                x = circuits["{}_x".format(modal)]
                edge_index = circuits["{}_edge_index".format(modal)]
                ...
                while cur_node_level < max_level:
                    for idx in level_list[cur_node_level]:
                        hop_nodes = get_hops(idx, edge_index, k_hop=K_HOP)
                ...
                for po_idx in po_list:
                    if po_idx not in hop_node_list:
                        hop_nodes = get_hops(po_idx, edge_index, k_hop=K_HOP)
            ...
            #mig:
            mig_edge_index =  torch.tensor(circuits["mig_edge_index"], dtype=torch.long).contiguous()
            mig_x = circuits["mig_x"]
            ...
            # graph.mig_tt_dis = circuits["mig_tt_dis"]
            # graph.mig_tt_pair_index = torch.tensor(circuits["mig_tt_pair_index"], dtype=torch.long).t().contiguous()
            graph.mig_x = torch.tensor(circuits["mig_x"])
            graph.mig_edge_index = torch.tensor(circuits["mig_edge_index"], dtype=torch.long).contiguous()
            # graph.mig_prob = torch.tensor(circuits["mig_prob"])
            ...
            #xag
            xag_edge_index = torch.tensor(circuits["xag_edge_index"], dtype=torch.long).contiguous()
            xag_x = circuits["xag_x"]
            ...
            # graph.xag_tt_dis = circuits["xag_tt_dis"]
            # graph.xag_tt_pair_index =  torch.tensor(circuits["xag_tt_pair_index"], dtype=torch.long).t().contiguous()
            graph.xag_x = torch.tensor(circuits["xag_x"])
            graph.xag_edge_index = torch.tensor(circuits["xag_edge_index"], dtype=torch.long).contiguous()
            # graph.xag_prob = torch.tensor(circuits["xag_prob"])
            ...
            graph.xag_gate = torch.tensor(circuits["xag_x"][:, 1:2], dtype=torch.float)

            #aig
            # graph.aig_tt_sim = circuits["aig_tt_sim"]
            # graph.aig_tt_pair_index =  torch.tensor(circuits["aig_tt_pair_index"], dtype=torch.long).contiguous()
            graph.aig_x = torch.tensor(circuits["aig_x"])
            graph.aig_edge_index = torch.tensor(circuits["aig_edge_index"], dtype=torch.long).contiguous()
            # graph.aig_prob = torch.tensor(circuits["aig_prob"])
            graph.aig_forward_index = torch.tensor(circuits["aig_forward_index"])
            graph.aig_forward_level = torch.tensor(circuits["aig_forward_level"])
            graph.aig_backward_index = torch.tensor(circuits["aig_backward_index"])
            graph.aig_backward_level = torch.tensor(circuits["aig_backward_level"])
            # graph.aig_gate = torch.tensor(circuits["aig_gate"])
            graph.aig_batch = torch.zeros(len(graph.aig_x), dtype=torch.long)
            graph.aig_gate = torch.tensor(circuits["aig_gate"], dtype=torch.float)
            
            #xmg
            graph.xmg_x = torch.tensor(circuits["xmg_x"])
            graph.xmg_edge_index = torch.tensor(circuits["xmg_edge_index"], dtype=torch.long).contiguous()
            graph.xmg_forward_level = torch.tensor(circuits["xmg_forward_level"])
            graph.xmg_forward_index = torch.tensor(circuits["xmg_forward_index"])
            graph.xmg_backward_level = torch.tensor(circuits["xmg_backward_level"])
            graph.xmg_backward_index = torch.tensor(circuits["xmg_backward_index"])
            graph.xmg_batch = torch.zeros(len(graph.xmg_x), dtype=torch.long)
            graph.xmg_gate = torch.tensor(circuits["xmg_x"][:, 1:2], dtype=torch.float)
            
            # # Prob 
            # graph.aig_prob = torch.tensor(aig_prob, dtype=torch.float)
            # graph.xmg_prob = torch.tensor(xmg_prob, dtype=torch.float)
            
            graph.name = self.circuit_name
...
#                 # if len(circuits) != 16:
#                 #     print(f"Skipping circuit {cir_name} with length {len(circuits)}")
...
#                 #mig:
#                 x = circuits["mig_x"]
#                 edge_index = circuits["mig_edge_index"]
#                 # is_pi = circuits["is_pi"]
#                 # no_edges = circuits["no_edges"]
#                 prob = circuits["mig_prob"]
#                 # backward_level = circuits["backward_level"]
#                 # forward_index = circuits["forward_index"]
#                 # forward_level = circuits["forward_level"]
#                 # no_nodes = circuits["no_nodes"]
#                 # backward_index = circuits["backward_index"]
#                 tt_dis = circuits["mig_tt_dis"]
#                 tt_pair_index = circuits["mig_tt_pair_index"]
...
#                 #xmg:
#                 xmg_edge_index =  torch.tensor(circuits["xmg_edge_index"], dtype=torch.long).t().contiguous()
#                 xmg_x = circuits["xmg_x"]
...
#                 graph.xmg_tt_dis = circuits["xmg_tt_dis"]
#                 graph.xmg_tt_pair_index = torch.tensor(circuits["xmg_tt_pair_index"], dtype=torch.long).t().contiguous()
#                 if len(graph.xmg_tt_pair_index) == 0:
#                     print('No tt : ', cir_name)
#                     continue
#                 graph.xmg_x = torch.tensor(circuits["xmg_x"])
#                 graph.xmg_edge_index = torch.tensor(circuits["xmg_edge_index"], dtype=torch.long).t().contiguous()
#                 graph.xmg_prob = torch.tensor(circuits["xmg_prob"])
...
#                 #xag
#                 xag_edge_index = torch.tensor(circuits["xag_edge_index"], dtype=torch.long).t().contiguous()
#                 xag_x = circuits["xag_x"]
#                 xag_forward_level, xag_forward_index, xag_backward_level, xag_backward_index = return_order_info(xag_edge_index, torch.LongTensor(xag_x).size(0))
#                 graph.xag_tt_dis = circuits["xag_tt_dis"]
#                 graph.xag_tt_pair_index =  torch.tensor(circuits["xag_tt_pair_index"], dtype=torch.long).t().contiguous()
...
#                     continue
#                 graph.xag_x = torch.tensor(circuits["xag_x"])
#                 graph.xag_edge_index = torch.tensor(circuits["xag_edge_index"], dtype=torch.long).t().contiguous()
#                 graph.xag_prob = torch.tensor(circuits["xag_prob"])
#                 graph.xag_forward_level = xag_forward_level
#                 graph.xag_forward_index = xag_forward_index
#                 graph.xag_backward_level = xag_backward_level
